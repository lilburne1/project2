...
class ConeDetector(Node):
    def __init__(self):
        ...
        self.web_image_publisher = self.create_publisher(Image, 'Cone_found_Web', 10)
    ...
    def detect_cone(self, msg):
        ...
        if detected:
            ...
            self.get_logger().info("Cone detected, publishing to web topic.")
            # Publish the current image to the web topic
            try:
                web_image_msg = self.bridge.cv2_to_imgmsg(bgr_image, encoding="bgr8")
                self.web_image_publisher.publish(web_image_msg)
                self.get_logger().info("Published image to Cone_found_Web topic.")
            except CvBridgeError as e:
                self.get_logger().error(f"Could not convert image for web: {e}")
